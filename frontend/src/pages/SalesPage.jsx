...
export default function SalesPage() {
    ...
    async function fetchAll() {
      ...
      try {
        const [saleData, productData] = await Promise.all([
          api.sales.list(token, { limit: 100 }),
  ...
  function saleTotal(sale) {
    return sale.items.reduce((sum, i) => sum + i.quantity * i.unit_price, 0);
  }
  ...
  async function handleSubmit(e) {
    ...
    setSaving(true);
    try {
      const created = await api.sales.create(token, payload);
      setSales((prev) => [created, ...prev]);
      setModalOpen(false);
    } catch (err) {
      // Surface the backend's insufficient-stock (or any other) rejection
      // message verbatim, rather than a generic "sale failed" string.
  ...
  return (
    <div className="list-page">
      ...
      {!isAdmin && (
        <p className="list-readonly-note">Read-only — admin role required to record a sale.</p>
      )}
      {isAdmin && products.length === 0 && !loading && (
        <p className="list-readonly-note">Add at least one product before recording a sale.</p>
      )}

      {loading ? (
        <p className="list-state">Loading sales…</p>
      ) : listError ? (
        <p className="list-state form-error">{listError}</p>
      ) : sales.length === 0 ? (
        <p className="list-state">No sales recorded yet.</p>
      ) : (
          ...
          <tbody>
            {sales.map((sale) => (
              <tr key={sale.id}>
                ...
                <td className="mono">${saleTotal(sale).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {modalOpen && (
        <Modal title="New sale" onClose={closeModal} wide>
